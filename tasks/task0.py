		...
		for i in range(len(task["betsect"])):
			if task["betsect"][i].label != "":
				...
				if task["betsect"][i].label_off == "справа-сверху":
					...
					self.painter.setPen(Qt.NoPen)
					self.painter.drawRect(QRectF(
						QPointF(self.wsect(i) -3 + 6,                                      hcenter - QFontMetrics(self.font).height()/2 ),
						QPointF(self.wsect(i) +3 + 6 + QFontMetrics(self.font).width(txt), hcenter + QFontMetrics(self.font).height()/2 )
					))
